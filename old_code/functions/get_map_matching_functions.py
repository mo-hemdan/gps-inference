...
def get_graph_from_bb(latitude, longitude, network_type, verbose=False):
    ...
    e = max(longitude)
    w = min(longitude)
    bbox = (w, s, e, n)  # left, bottom, right, top

    G = ox.graph_from_bbox(bbox=bbox, network_type=network_type, retain_all=True, truncate_by_edge=True, simplify=False) # simplify=True, 
...
def compute_OSM_node_vals(x, vals_list, nodes, unique_n_dict):
...
def compute_OSM_edge_vals(x, vals_list, edges):
