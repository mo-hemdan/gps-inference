...
class EditedAnaMetadataInference__temp():
        
    def __init__(self):
        pass   

    def map_match(self, df=None, file_path=None, save=True):
    ...
    def extract_detailed_metadata(self, matched_df, trip_veh_id_str, timestamp_str, lat_str, lon_str ):
    ...
    def TIME_agg(self, matched_df, timestamp_str):
        def time_expand(row):
        ...
        return matched_df_w_t

    def group_extracted_metadata(self, matched_df_w_t, trip_veh_id_str):
        ...
        return way_metadata, node_metadata
    
    def read_file(self, file_path, file_type, compression):
        return pd.read_csv(file_path, index_col=0)


    def extract_metadata(self, file_path, file_type="csv", compression=None, save=True):
        # Load the trajectory data
        # input_path = "/project/cs-dmlab/areeg/Metadata/metadata_inference/data/"
        # file = "top_10_percentage.csv.csv"
        df = self.read_file(file_path, file_type, compression)
        ...
        matched_df = self.map_match(df, lat_str, lon_str)
        print("matching done")
        ...
        matched_df = self.extract_detailed_metadata(matched_df, trip_veh_id_str, timestamp_str, lat_str, lon_str)
        print("extracting done")

        #%%
        matched_df_w_t = self.TIME_agg(matched_df, timestamp_str)
        print("time expanded")

        #%%
        ######### Extract metadata given matched edges and nodes ###########
        way_metadata, node_metadata = self.group_extracted_metadata(matched_df_w_t, trip_veh_id_str)

        print("aggregating done")
        return way_metadata, node_metadata  # Where way_metadata is the dataframe for metadata for edges while node_metadata for metadata for nodes. 
