...
result = df.groupby('osmid')['OSM_oneway'].apply(resolve_oneway).reset_index()

# %%
result.rename(columns={'osmid': 'id', 'OSM_oneway': 'osm_oneway_direction'}, inplace=True)
...
query = """
UPDATE roads
SET osm_oneway_direction = data.osm_oneway_direction
FROM (VALUES %s) AS data(id, osm_oneway_direction)
